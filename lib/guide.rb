...
class Guide
  ...
  def initialize(path=nil)
    #Locate the restaurant text file @ path
    Restaurant.filepath = path
    if Restaurant.file_usable?
      puts "Found restaurant file!"
      #or create a new file
    elsif Restaurant.create_file
      puts "Createded the restaurant file"
    else
  ...
  def get_action
    action = nil
    #Keep asking for user input until gets a valid action
    if Guide::Config.actions.include?(action) == false
  ...
  def do_action(action, args=[])
    case action
      ...
      when 'quit'
        ...
        puts "\nThere is no such command. Please use commands from the ones below!\n"
    end
  end
